...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $newTitle = $_POST["bn_title"];
    $newDescription = $_POST["bn_desc"];
    $newSort = $_POST["sort"];
    $newBtnText = $_POST["btn_text"];
    ...
    if (isset($_FILES["image"]) && $_FILES["image"]["error"] == 0) {
        ...
        if (is_uploaded_file($_FILES["image"]["tmp_name"])) {
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $newImagePath)) {
                ...
                $query = "UPDATE `banner` SET Title = ?, Description = ?, btntext = ?, sortorder = ?, Image = ? WHERE id = ?";
                ...
                if (!$stmt) {
                    ...
                    $stmt->bind_param("sssssi", $newTitle, $newDescription, $newBtnText, $newSort, $newImageName, $user_id);
        ...
        $query = "UPDATE `banner` SET Title = ?, Description = ?, btntext = ?, sortorder = ? WHERE id = ?";
        $stmt = $Connection->prepare($query);
        ...
        if (!$stmt) {
            ...
            $stmt->bind_param("ssssi", $newTitle, $newDescription, $newBtnText, $newSort, $user_id);
